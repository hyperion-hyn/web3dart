...
class Bech32Codec extends Codec<Bech32, String> {
  ...
  String encode(Bech32 data, [int maxLength = Bech32Validations.maxInputLength]) {
    return Bech32Encoder().convert(data, maxLength);
  }

  Bech32 decode(String data, [int maxLength = Bech32Validations.maxInputLength]) {
...
class Bech32Encoder extends Converter<Bech32, String> with Bech32Validations {
  String convert(Bech32 input, [int maxLength = Bech32Validations.maxInputLength]) {
    ...
    if (hrp.length + data.length + separator.length + Bech32Validations.checksumLength > maxLength) {
...
String ethAddressToBech32Address(String ethAddress) {
  if (ethAddress != null && ethAddress.startsWith('hyn1')) {
    return ethAddress;
  }

  if (ethAddress != null && ethAddress.isNotEmpty) {
    final address = EthereumAddress.fromHex(ethAddress);
    final hynAddressSegwit = Segwit('hyn', 1, address.addressBytes);
    return segwit.encode(hynAddressSegwit);
  }
  return '';
}

String bech32ToEthAddress(String bech32Address) {
  if (bech32Address != null && !bech32Address.startsWith('hyn1')) {
